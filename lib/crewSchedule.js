...
// `notBefore` is when this schedule started applying. A slot earlier than that
// cannot have been missed, because at the time it was not a slot.
//
// Without it, widening a cadence accuses everyone retroactively. Adding Saturday
// and Sunday to Joy, Reef and Shelly on a Sunday evening instantly made all
// three "quiet" for a 9am Sunday run that did not exist at 9am that morning —
// a fresh crop of false alarms caused by fixing false alarms.
function isQuiet(run, schedule, now, graceMs, notBefore) {
  ...
  if (!due) return false;
  if (notBefore != null) {
    const from = notBefore instanceof Date ? notBefore.getTime() : new Date(notBefore).getTime();
    if (Number.isFinite(from) && due.getTime() < from) return false;
  }
