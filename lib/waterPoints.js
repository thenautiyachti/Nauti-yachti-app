...
// "asked" means somebody put the question and got an answer that was not a
// refusal, or has an understanding. "no" is a hard exclusion.
const PERMISSION = ["yes", "no", "asked"];
...
function validate(input) {
  ...
  // The four things that decide whether a borrowed slip is usable. Unknown is
  // a real answer and stays null — "nobody has checked" is not "no".
  const tri = (v) => (v === true || v === "true" ? true : v === false || v === "false" ? false : null);
  const covered = tri(input && input.covered);
  const onProperty = tri(input && input.onProperty);
  const motorized = tri(input && input.motorized);
  let permission = null;
  if (input && input.permission) {
    if (!PERMISSION.includes(input.permission)) errors.push("Permission must be yes, no or asked.");
    else permission = input.permission;
  }
  ...
  return {
    ...
    value: errors.length ? null : {
      name, kind, lat, lon,
      radiusYards: radius,
      covered, onProperty, motorized, permission,
...
module.exports = {
  KINDS, PLACE_KINDS, HAZARD_KINDS, KIND_LABEL, KIND_ICON, PERMISSION,
