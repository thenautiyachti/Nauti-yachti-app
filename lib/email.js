...
// WHICH BOAT THIS BOOKING IS ON, as a key.
//
// The row may carry the id, or only the name: the mirror ExternalBooking rows
// and the object the webhook builds by hand have each been one or the other.
// Name matching is the fallback and is a substring, because both "Nauti
// Explorer" and "Explorer" have been written on a row.
function vesselKey(booking) {
  const b = booking || {};
  const id = String(b.vesselId || "").trim().toLowerCase();
  if (id) return id;
  const name = String(b.vesselName || "").toLowerCase();
  return ["explorer", "islander", "yachti"].find((k) => name.includes(k)) || "";
}

// WHERE A BOAT LIVES IS A FACT ABOUT THE BOAT, NOT ABOUT THE BUSINESS.
//
// Owner, 18 Sep 2026: "for the explorer pearl Bay is the address. And the other
// two boats have the other dock location."
//
// prisma/schema.prisma has known this for as long as dockLat/dockLon have
// existed — the Explorer sits at Pearl Bay on the east side, the Islander and
// the Yachti three miles WSW — and the nowcast already asks per boat, because
// "a single DOCK_LAT/DOCK_LON would have routed two of the three fleet to the
// wrong shore". The ADDRESS never got the same treatment. One DOCK_ADDRESS went
// to every guest whichever boat they had booked, so two thirds of the fleet
// have been mailing people to a house on the wrong side of the lake.
//
// Addresses stay in the environment and out of the repository: both docks are
// private residences. DOCK_ADDRESS keeps its meaning — the Explorer's, which is
// what it has always held — and the others read DOCK_ADDRESS_<ID>.
//
// THERE IS NO FALLBACK BETWEEN DOCKS, deliberately. With the Islander's address
// unset the email degrades to "we'll be in touch with the meeting point", which
// is true and costs one text. Falling back to DOCK_ADDRESS would quietly
// reinstate the exact bug this replaces, and a confidently wrong address is the
// one failure a guest cannot catch before they are already driving.
function dockAddressFor(booking) {
  const key = vesselKey(booking);
  if (!key) return "";
  const own = process.env["DOCK_ADDRESS_" + key.toUpperCase()];
  if (own && own.trim()) return own.trim();
  return key === "explorer" ? String(process.env.DOCK_ADDRESS || "").trim() : "";
}

// What is true about a particular dock, kept beside the address rather than
// written into the email as though it were true of all of them.
//
// Pearl Bay is gated — that is why DOCK_GATE_CODE exists and why the email
// promises a text on the morning. Whether the west dock is has not been said,
// and promising somebody a gate code that does not exist is its own small
// betrayal, so the promise is not made until it is. Same for the description:
// "no sign, no dock office, parking on site" describes Pearl Bay. It is not
// claimed about a dock nobody has described.
const DOCK_FACTS = {
  explorer: {
    gated: true,
    note: "It is a private gated dock rather than a public marina, so there is no sign "
...
async function sendBookingConfirmationEmail(booking) {
  ...
  const dockAddress = dockAddressFor(booking);
  const dockFacts = DOCK_FACTS[vesselKey(booking)] || {};
  ...
  const html = `
    <div style="font-family:system-ui,-apple-system,Segoe UI,sans-serif;max-width:520px">
      ...
      <div style="margin:22px 0 0;padding:16px 18px;background:#faf7fd;border:1px solid #e6dcf0;border-radius:8px">
        ...
        ${dockFacts.note ? `<p style="margin:12px 0 0;font-size:13px;color:#555">${dockFacts.note}</p>` : ""}
        ${!dockFacts.gated ? "" : gateCode
