  ...
  return all
    ...
    .map((c) => {
      ...
      return {
        ...
        platform: c.platform,
        postId: c.postId,
        // The PLATFORM's own id for the post, which Blotato returns on every
        // comment and this file used to drop on the floor. It is what builds a
        // Facebook permalink, and it is the fallback when a comment cannot be
        // joined to a MediaDraft at all -- which is the normal case for anything
        // the owner posted by hand from his phone.
        //
        // Instagram deliberately gets no permalink built from it: an IG post URL
        // carries a shortcode, not the media id, so anything constructed here
        // would 404. Same doctrine as the TikTok line -- say "no link" rather
        // than hand somebody a broken one.
        platformPostId: c.platformPostId || null,
