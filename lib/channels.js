...
//
// "I will try" is the important half. This is a policy he is adopting, not a
// switch that has been thrown — an autopay he has forgotten about will keep
// charging Wells Fargo whatever anybody intends. So nothing here BLOCKS a Wells
// Fargo row. It warns, and it counts, which is what makes the drift visible
// while he is still moving direct debits across.
//
// Rows dated before the cutover are ordinary history and are never flagged:
// there are 171 Wells Fargo rows going back to June 2025 and every one of them
// is correct.
const RETIRING_ORIGINS = ["Wells Fargo Statement"];
const PREFERRED_ORIGIN = "Woodforest Statement";
const RETIRING_FROM = "2026-09-16"; // the day the decision was made

// True when a row is on the retiring account AND dated on or after the cutover.
// Takes the date as the "YYYY-MM-DD" string LedgerEntry stores, so no timezone
// ever enters into it.
function isRetiringOrigin(origin, date) {
  if (!RETIRING_ORIGINS.includes(origin)) return false;
  if (!date) return true; // undated on a retiring account is worth a look either way
  return String(date) >= RETIRING_FROM;
}

// The sentence shown wherever one of these turns up. One wording, so the form,
// the checker and any agent that reports it all say the same thing.
function retiringOriginWarning(origin) {
  return `${origin} is being retired. Since ${RETIRING_FROM} the business pays from `
    + `${PREFERRED_ORIGIN} (Woodforest Business Checking). If this really did come out `
    + `of Wells Fargo, file it — then move whatever is still charging that account.`;
}

module.exports = {
  LEAD_SOURCES, BOOKING_CHANNELS, PAYMENT_METHODS, LEDGER_ORIGIN,
  normaliseChannel, normaliseLeadSource, ledgerOriginFor,
  RETIRING_ORIGINS, PREFERRED_ORIGIN, RETIRING_FROM,
  isRetiringOrigin, retiringOriginWarning,
};
