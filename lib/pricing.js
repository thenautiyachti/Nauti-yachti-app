const { addOnTotal, takesAddOns } = require("./addOns");
...
function quoteTotal(pkg, addOns, opts) {
  ...
  // A per-seat event takes no add-ons — see takesAddOns in lib/addOns.js. The
  // check lives HERE rather than only in the booking form because this is the
  // function that decides what Stripe charges. Slade Deliberto was billed $60
  // for a decoration package on two glow seats on 18 Sep 2026: the form offered
  // it, and nothing between the form and the card disagreed.
  //
  // Ignored rather than refused. A guest who has ticked a box should still be
  // able to check out; they simply are not charged for something that cannot be
  // delivered, and the booking records no add-on against them.
  const extras = takesAddOns(pkg)
    ? addOnTotal(addOns || [], pkg.id, (opts && opts.addOnIds) || [])
    : 0;
...
// How long a package runs, as a guest should read it.
//
// fixedHours is an integer because it is a number, and a package whose length
// genuinely varies cannot be one. Boatz & Glowz is 7 hours as of 17 Sep 2026:
// rope off at 5pm and back around midnight. It has been 4, then 5, then 7 in a
// single day, which is the argument for reading it from the row rather than
// writing it into a sentence — the page once said four hours while the times
// printed beside it added up to five.
//
// So durationLabel wins when it is set, and everything falls back to the
// number. Both forms end in the word hours, so callers never add it.
function durationText(pkg) {
  if (!pkg) return null;
  const label = String(pkg.durationLabel || "").trim();
  if (label) return label;
  if (pkg.fixedHours == null) return null;
  return pkg.fixedHours + (Number(pkg.fixedHours) === 1 ? " hour" : " hours");
}

module.exports = { currency, tierPrice, dayTypeForDate, quotePackage, quoteTotal, localDateKey, imageFocus, durationText };
