...
// Read a coordinate off a query string. ABSENT IS NOT ZERO.
//
// `URLSearchParams.get()` returns null for a missing parameter and `Number(null)`
// is 0, which is finite — so a plain `Number.isFinite` check let a missing
// position through as 0. On 6 Sep 2026 a phone that could not get a fix sent no
// coordinates, the server read the boat as sitting at 0°N 0°E, and the page
// reported "Dock is 6551.1 mi WNW, about 19654 min at cruise" over a forecast
// fetched for the Gulf of Guinea and presented as this lake.
//
// `Number("")` is 0 as well, so an empty parameter is the same trap.
...
// Is this a position a boat of ours could actually be at?
//
// A second layer, because the first one already failed. Whatever the route in —
// a parsing slip, a phone with a broken fix, a stale position cached on another
// continent — a boat on Lake Conroe is not thousands of miles from Lake Conroe,
// and the page must refuse rather than compute confidently from it.
//
// Deliberately wide: 300 miles admits the owner opening this from home or with
// a boat on a trailer anywhere in the region, and admits no other continent.
const REACH_MILES = 300;

function withinReach(p, of, maxMiles) {
  if (!p || !Number.isFinite(p.lat) || !Number.isFinite(p.lon)) return false;
  if (p.lat < -90 || p.lat > 90 || p.lon < -180 || p.lon > 180) return false;
  const d = milesBetween(p, of);
  return d != null && d <= (maxMiles == null ? REACH_MILES : maxMiles);
}

module.exports = {
  milesBetween, bearingTo, compass, sampleLine,
  minutesUntilWet, minutesHome, assess,
  parseCoord, withinReach,
  WET_MM, MPH_PER_KNOT, REACH_MILES,
};
