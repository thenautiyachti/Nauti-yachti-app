...
// A PER-SEAT EVENT TAKES NO ADD-ONS AT ALL.
//
// Owner, 18 September 2026: "The boats and glow package shouldn't have any add
// ons. This would just be by itself."
//
// The reason is the shape of the thing being sold. Every other package hands a
// guest the whole boat, so balloons on the bow and champagne on ice are theirs.
// Boatz & Glowz sells ONE SEAT on a boat carrying a dozen strangers -- there is
// no "your table" to decorate, and setting up for two people in the middle of
// somebody else's night is not a thing that can be delivered.
//
// It went out the door before it was caught. Slade Deliberto checked out on the
// morning of 18 September for two glow seats and picked up the Full Decoration
// Package, taking $100 to $160 for something nobody could have set up. He had
// to be texted and refunded.
//
// KEYED ON pricingType, NOT ON A LIST OF PACKAGE IDS. "per-guest" IS the
// definition of a per-seat event, so the next one inherits this on the day it
// is created rather than on the day somebody remembers to add it to a list.
function takesAddOns(pkg) {
  if (!pkg) return true;
  return pkg.pricingType !== "per-guest";
}
