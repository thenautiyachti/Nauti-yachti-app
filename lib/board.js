...
// The item as first written, with its accreted note trail removed. formatBody
// already knows where that trail begins, so the boundary is defined in exactly
// one place instead of being guessed at in two.
function claimOf(body) {
...
function priorityOf(text) {
  ...
  // Rank on the CLAIM, never on the notes underneath it. Every verification
  // note carries the date it was written -- "CHECKED 2026-09-04" -- and
  // daysUntil() reads the first ISO date in the body as a deadline. On 4
  // September that put eight open items in HIGH purely because somebody had
  // checked them that morning: the board ranking attention rather than urgency,
  // and getting worse the more the crew did their job.
  const claim = claimOf(body);

  const due = daysUntil(claim);
  ...
  if (HIGH.test(claim)) return "high";
  if (imminent) return "high";
  if (MEDIUM.test(claim) || soon) return "medium";
  return "low";
}
