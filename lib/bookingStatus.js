...
// platform inquiry that never converted, and -- on the Inquiry side -- a website
// inquiry that went quiet, which the bucket map literally spelled
// `lapsed: "cancelled"`.
//
...
// direction nobody could see. "Every GetMyBoat inquiry since 13 June cancelled"
...
// without fixing those first would have put 33 inquiries on the live calendar as
...
// Website inquiries carry their own status values. Both models now land on the
// same five buckets, so the unified table can stop pretending a lapsed inquiry
// is a cancellation.
const INQUIRY_STATUS_BUCKET = {
  new: "inquiry",
  // "pending" is NOT issued by anything. Counted 11 Sep 2026: zero rows carry
  // it, in either table. It stays mapped because an old row could still
  // surface and the mapping costs nothing — but do not reach for it as a
  // status, and do not read a "pending" in the console as one.
  //
  // WHERE A VISIBLE "pending" ACTUALLY COMES FROM: toUnifiedRows falls back to
  // the literal string "pending" for any status missing from this map. So a row
  // showing that bucket is an UNRECOGNISED status, not a pending one. The two
  // Inquiry rows carrying "seen" are the live example — and "seen" is
  // deliberately absent below, because those rows are crew-list email signups
  // (packageId "crewlist"), which isRealInquiry already keeps out of the
  // bookings table entirely. They are mailing-list contacts, not leads, and
  // mapping their status here would invite them back in as bookings.
  pending: "inquiry",
  lapsed: "lapsed",
  booked: "booked",
  owed: "owed",
...
// inquiry, and what makes a repayment possible. True for an owed charter for
...
// Somebody asked. Live inquiries and lapsed ones together are the denominator
// for a conversion rate; neither is the numerator for anything.
function isInquiry(status) {
...
// undelivered charter, not knowing how much is a gap by any reading. An inquiry
// with no price is not a gap, it is an inquiry.
...
function looksLikeALapsedInquiry(booking) {
  if (!booking) return false;
  if (booking.status !== "cancelled") return false;
  return !Number(booking.pricePaid || 0);
}

// --- ONE CHARTER, TWO ROWS ---------------------------------------------------
//
// A website checkout writes an Inquiry AND a mirror ExternalBooking (see the
// Stripe webhook). Nothing kept their statuses together, so marking the charter
// completed in Bookings left the Inquiry saying "booked and paid" forever.
//
// Oscar RoblesGil's 6 Sep charter is the row that showed it: booking
// "completed", inquiry still "booked". He was the FIRST website checkout to
// produce such a pair, which is the only reason it had not been noticed — it
// would have happened to every one after him.
//
// These translate between the two vocabularies, which differ in exactly one
// place: a live inquiry is "inquiry" on a booking and "new"/"pending" on an
// Inquiry row. Everything else is the same word in both.

// Is the pair already saying the same thing? Compare BUCKETS, never strings —
// otherwise a perfectly good "pending" gets rewritten to "new" on every save.
function statusesAgree(inquiryStatus, bookingStatus) {
  return INQUIRY_STATUS_BUCKET[inquiryStatus] === bookingStatus;
}

// Booking status -> what the Inquiry row should say.
function bookingStatusToInquiry(bookingStatus) {
  return bookingStatus === "inquiry" ? "new" : bookingStatus;
}

// Inquiry status -> what the booking row should say. This is just the bucket
// map, named so the calling code reads in the direction it is going.
function inquiryStatusToBooking(inquiryStatus) {
  return INQUIRY_STATUS_BUCKET[inquiryStatus] || null;
}

// For Prisma `in` filters, where a predicate cannot be used. Derived from the
// predicates so the two cannot drift apart.

// WHAT THE STATUS COLUMN SHOULD ACTUALLY SAY.
//
// Owner, 18 September 2026, looking at the glow night list: "we have a list of
// those who are booked and paid, and those who are booked and also unpaid...
// can we change booked unpaid to leave it as blue and booked to green, which
// would be paid... we can have a book and payment failed so let's make that one
// red."
//
// He is describing a real gap. `booked` is one word covering two opposite
// situations: a charter that is settled and needs nothing, and a charter where
// somebody still owes money and nobody is chasing it. On the morning he asked,
// his own glow night had five BOOKED rows of which exactly one had been paid,
// and the column said the same thing about all five.
//
// PAYMENT IS A SEPARATE AXIS FROM STATUS, and this is the only place they are
// combined. The lifecycle stays six values; this decides how one of them LOOKS
// once payment is taken into account. Nothing here changes what a booking is.
//
// Only `booked` splits. An inquiry is unpaid by definition -- that is what makes
// it an inquiry -- so colouring it by payment would say nothing. Completed,
// owed, lapsed and cancelled keep their own meaning.
const PAYMENT_COLORS = {
  paid: "#7FE0B8",    // the console's existing green, as used for completed
  unpaid: "#4FA8E8",  // the blue booked already had, so nothing moves for unpaid
  failed: "#E2685F",  // the red a negative bank balance uses
};

/**
 * Label and colour for a row's status cell.
 *
 * Takes the whole row rather than a status string, because the answer depends
 * on the payment columns as well. Returns { label, color, tone } where tone is
 * "paid" | "unpaid" | "failed" | null, so a caller can branch without matching
 * on a colour.
 */
function bookingDisplay(row) {
  const r = row || {};
  const bucket = r.statusBucket || r.status;
  const label = LABELS[bucket] || bucket || "";
  const base = { label, color: COLORS[bucket] || COLORS.inquiry, tone: null };

  if (bucket !== "booked") return base;

  // A card that was declined outranks everything else on the row: it is the one
  // state where the guest has tried to give us money and failed, and it needs a
  // different message from somebody who simply has not paid yet. Cleared the
  // moment a payment succeeds, so a paid booking never wears a stale decline.
  if (r.paymentFailedAt && r.paymentStatus !== "paid") {
    return { label: "Booked / payment failed", color: PAYMENT_COLORS.failed, tone: "failed" };
  }

  if (r.paymentStatus === "paid") {
    // A booking at no charge is settled but is not a sale, and calling it paid
    // puts the crew riding free in the same sentence as a guest who handed over
    // money. Same green -- nothing is owed either way.
    const amount = r.pricePaid != null ? r.pricePaid : r.priceQuoted;
    if (!(Number(amount) > 0)) {
      return { label: "Booked / no charge", color: PAYMENT_COLORS.paid, tone: "paid" };
    }
    return { label: "Booked / paid", color: PAYMENT_COLORS.paid, tone: "paid" };
  }

  return { label: "Booked / unpaid", color: PAYMENT_COLORS.unpaid, tone: "unpaid" };
}

const HOLDS_THE_DAY = STATUSES.filter(holdsTheDay);
const INQUIRIES = STATUSES.filter(isInquiry);
