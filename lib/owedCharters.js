...
// `today` as a "YYYY-MM-DD" string, whatever shape it arrived in.
//
// This module compares dates as strings and lib/reviews.js daysSince works in
// Dates, and callers have handed over both. A Date reaching the `d < today`
// comparison below is the quiet half of that: relational comparison converts
// the Date to a number and the date string to NaN, every comparison is false,
// and the owed list comes back EMPTY — no error, no row, a guest who paid for
// a charter that never ran simply absent from the one screen built to find him.
// The loud half took the console down on 20 Sep 2026; see daysSince.
...
function isOwedCharter(b, today) {
  ...
  // Since 5 Sep 2026 this state has a status of its own, and the label alone is
  // enough. Note it is checked BEFORE the price guard below, deliberately: an
  // owed charter whose payment was never recorded is the exact row this module
  // exists to stop losing, and making it prove it paid first would lose it
  // again. If the owner has said the business owes this guest a boat, it does.
  if (b.status === "owed") return true;

  // Everything below is the inference that had to stand in before the status
  // existed, and still has to for rows nobody has relabelled: money taken, date
  // gone or never set, charter never delivered.
  if (Number(b.pricePaid) <= 0) return false;
  const d = String(b.date || "");
  const dated = /^\d{4}-\d{2}-\d{2}$/.test(d);
  return !dated || d < dayKey(today);
}
