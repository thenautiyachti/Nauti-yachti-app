...
// The Facebook Page's Recommendations tab. Note this is the profile.php id the
// public site already links to (61577960573366) and NOT the pageId in
// lib/socialPosting.js (630671406805108) -- that one is what the posting API
// addresses the page by, and it does not resolve as a facebook.com URL.
//
// Verified on 2026-09-05: this URL loads The Nauti Yachti's page on the
// recommendations tab. It currently reads "No reviews", which is the point.
//
// Google stays the primary ask everywhere. A Google review shows up in the
// search result a stranger sees when they look for boat rental on Lake Conroe;
// a Facebook recommendation is only seen by someone already on the page.
const FACEBOOK_REVIEW_URL = "https://www.facebook.com/profile.php?id=61577960573366&sk=reviews";
...
function fmtLooseDate(dateStr, today) {
  ...
  // Same Date-or-string hazard as daysSince below, reached from reviewMessage
  // with the same value. Fixed here at the same time rather than waiting for it
  // to take a screen down on its own.
  const now = today instanceof Date ? today : (parseDay(today) || new Date());
...
//
// `today` ARRIVES AS BOTH A DATE AND A STRING and always has. Most callers hand
// over a Date; lib/owedCharters.js hands over the "YYYY-MM-DD" string it needs
// for its own `d < today` comparison, and the console's OverviewTab builds that
// string with localDateKey(new Date()) before passing it down. Calling
// .getFullYear() on a string throws, so this now takes either.
//
// IT TOOK DOWN THE WHOLE CONSOLE ON 20 SEP 2026. owedCharters only reaches this
// function for an owed charter that HAS a real date — Christian Gehring's said
// "TBD", so for weeks the only qualifying row skipped the call and the fault sat
// dormant. On the morning of the 20th Saturday's paid glow charters slipped into
// the past still unmarked, qualified as owed WITH real dates, and every render of
// the overview tab threw "getFullYear is not a function". React retried, threw
// again, and Chrome killed the tab — which reads as "this page couldn't load"
// and looks for all the world like a login problem.
