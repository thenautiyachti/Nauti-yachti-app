...
// A stamp opens with a shout: CLOSED, CHECKED, RECOUNTED, OWNER CONFIRMED,
// CAUSE ESTABLISHED, HALF SHIPPED. Listing the verbs by hand did not survive
// contact with the crew -- within a day the list was missing RECOUNTED,
// QUANTIFIED and OWNER CONFIRMED, so those notes were never recognised as notes.
// They stayed inside the claim, where they polluted both what the item appeared
// to say and how it was ranked.
//
// So the rule is the SHAPE, not a vocabulary: one to four capitalised words,
// then a date. It cannot fall behind the crew's wording, and the date plus the
// sentence-start anchor is what keeps it from firing on ordinary prose.
const VERBS = "[A-Z][A-Z]+(?:\\s+[A-Z][A-Z]+){0,3}";
...
const STAMP = new RegExp(
  "^(?:(\\b(?:" + VERBS + ")\\b(?:\\s+[A-Z][A-Z]*)*(?:\\s+by\\s+[A-Z][a-z]+)?\\s+(?:on\\s+)?" + DATEISH + ")" +
    ...
    // The trailing separator once demanded whitespace, so "MEASURED 2026-09-04
    // by Pearl. The reporting side..." -- an attribution ending in a full stop
    // rather than a comma -- failed to capture "by Pearl", which then appeared
    // glued to the date at the head of the note instead of in its chip.
    // A full stop is a separator here too, and none of it is required.
    "\\s*(?:by\\s+([A-Z][a-z]+))?\\s*[:,.\\u2014\\u2013-]*\\s*"
);
