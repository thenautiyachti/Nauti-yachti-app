...
// show the whole photo (letterboxed) instead of cropping. A couple are then
// zoomed in slightly on top of that (via IMAGE_ZOOM below) to trim some of
// the resulting letterbox dead space without cropping the subject out.
const IMAGE_CONTAIN = [
...
// Scale applied on top of object-fit:contain for specific photos, to eat
// into the letterbox bars a bit. 1 = no zoom (full photo visible).
const IMAGE_ZOOM = {
  "5fb07f44-df05-47ed-b223-a478b9cd2b48": 1.3, // Balloons on deck
  "35f17a13-71bb-4629-ad47-09c4a665f192": 1.25, // Making memories on the water
};
function imageZoom(url) {
  if (!url) return 1;
  const hit = Object.keys(IMAGE_ZOOM).find((id) => url.includes(id));
  return hit ? IMAGE_ZOOM[hit] : 1;
}

module.exports = { currency, tierPrice, dayTypeForDate, localDateKey, imageFocus, imageFit, imageZoom };
