...
/**
 * The same thing, found by its human reference: "NY-20260919-11".
 *
 * NOT for a /pay/<id> link — a reference is sequential and can be walked to
 * read other people's charters, which is exactly why the guest-facing link uses
 * the cuid. This is for the OWNER's tools, where the reference is what he has in
 * front of him and he is already authenticated.
 *
 * Inquiry first: when both tables hold the reference it is a website checkout,
 * and the inquiry is the row carrying the payment columns.
 */
async function findPayableByRef(ref) {
  if (!ref) return null;
  const reference = String(ref).trim().toUpperCase();

  const inquiry = await prisma.inquiry.findFirst({ where: { bookingId: reference } }).catch(() => null);
  if (inquiry) return withPricing(normalise(inquiry, "inquiry"));

  const external = await prisma.externalBooking.findFirst({ where: { bookingId: reference } }).catch(() => null);
  if (external) return withPricing(normalise(external, "external"));

  return null;
}

/** Which table a payable belongs to, so a caller never has to branch. */
function modelFor(b) {
  return b.kind === "external" ? prisma.externalBooking : prisma.inquiry;
}
...
module.exports = { findPayable, findPayableByRef, notPayableReason, metadataFor, modelFor };
