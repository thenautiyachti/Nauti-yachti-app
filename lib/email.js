...
async function sendInquiryEmail(entry) {
  ...
  // A CREW-LIST SIGNUP IS NOT A CHARTER ENQUIRY, and calling it one produced
...
//
// Before this, three of the four were one-sided. An enquiry and a crew-list
  ...
  return `
    <div style="font-family:system-ui,-apple-system,'Segoe UI',sans-serif;max-width:560px;margin:0 auto;color:#1a1a1a">
      ...
      <p style="margin:18px 0 0;font-size:12px;color:#888">
        The Nauti Yachti &middot; Lake Conroe, TX &middot; ${CONTACT_PHONE}
      </p>
    </div>`;
}
...
// triggered it — the enquiry, the signup or the payment is already saved, and
...
// 1. WHAT A GUEST GETS AFTER SENDING AN ENQUIRY.
//
// Deliberately promises a person, not a process. Somebody enquiring about a
...
async function sendInquiryAckEmail(entry) {
  ...
  return send({
    ...
    subject: "We've got your enquiry — The Nauti Yachti",
    label: "inquiry acknowledgement",
    html: guestShell(`
      <h2 style="margin:0 0 12px;font-size:20px">Thanks, ${first} &mdash; we've got it.</h2>
      <p style="margin:0 0 14px;font-size:14px;line-height:1.6">
        Your enquiry about <strong>${entry.packageName || "a charter"}</strong> has come
        through and Austin will come back to you personally, usually the same day.
      </p>
      <p style="margin:0 0 14px;font-size:14px;line-height:1.6">
