...
async function sendBookingConfirmationEmail(booking) {
  ...
  //
  // DOCK_GATE_CODE is still read here so the decision can be reversed in one
  // environment variable rather than a code change. Leaving it unset is the
  // intended state.
  //
  // Both come from the environment, never the repository: a gate code committed
  // to git is in its history permanently, and rotating the code would not
  // remove it. If unset, the block degrades to the old wording rather than
  // emailing the word "undefined" to a paying guest.
  const dockAddress = process.env.DOCK_ADDRESS || "";
  const gateCode = process.env.DOCK_GATE_CODE || "";
