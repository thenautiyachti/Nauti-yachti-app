 ...
 *
 * Deliberately NOT emitted: aggregateRating, and NOT because of the review
 * count. An earlier version of this note said it was waiting on more than one
 * review, which implied a threshold that does not exist and nearly caused
 * someone to switch it on at five.
 *
 * The real reason, verified against Google's docs on 3 Sep 2026:
 *
 *   - LocalBusiness: aggregateRating "is only recommended for sites that
 *     capture reviews about OTHER local businesses."
 *   - Review snippet policy: "If the entity that's being reviewed controls the
 *     reviews about itself, their pages are ineligible for star review
 *     feature." Self-serving reviews are no longer shown for LocalBusiness or
 *     Organization at all.
 *   - Google states NO minimum review count. This would be ineligible at 5
 *     reviews and at 500.
 *
 * So this never becomes eligible, and there is nothing to revisit. Star
 * ratings in local search come from the Google Business Profile, which needs
 * nothing from us.
 *
 * The same reasoning rules it out on the package Product nodes: a charter is
 * our own service, so a testimonial about it is a review of this business.
 * Product markup is the known workaround and Google knows it is the known
 * workaround; it is not worth a sitewide manual action for stars on a package
 * page. Do not add it there either.
 */
