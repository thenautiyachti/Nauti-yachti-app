const { isQuiet, nextDueRun } = require("./crewSchedule");
...
const CREW = [
  ...
  },
  {
    ...
    taskId: "nauti-coral",
    schedule: "Every day, 8:30am and 2:30pm",
  ...
  },
  {
    ...
    rank: "Chief Steward",
    taskId: "nauti-joy",
    schedule: "Fridays, Saturdays, Sundays and Mondays, 9am",
    // Widened on 6 Sep 2026. Slots before this are not missed runs — they
    // were not slots yet. See lib/crewSchedule.js.
    scheduleChangedAt: "2026-09-06T20:30:00",
  ...
  },
  {
    ...
    rank: "Navigator",
    taskId: "nauti-reef",
    schedule: "Fridays, Saturdays, Sundays and Mondays, 9:30am",
    // Widened on 6 Sep 2026. Slots before this are not missed runs — they
    // were not slots yet. See lib/crewSchedule.js.
    scheduleChangedAt: "2026-09-06T20:30:00",
  ...
  },
  {
    ...
    rank: "Quartermaster",
    taskId: "nauti-shelly",
    schedule: "Fridays, Saturdays, Sundays and Mondays, 10am",
    // Widened on 6 Sep 2026. Slots before this are not missed runs — they
    // were not slots yet. See lib/crewSchedule.js.
    scheduleChangedAt: "2026-09-06T20:30:00",
  ...
  },
  {
    ...
    rank: "Lookout",
    taskId: "nauti-nova",
    schedule: "Every day, 10:30am",
    // Widened on 6 Sep 2026. Slots before this are not missed runs — they
    // were not slots yet. See lib/crewSchedule.js.
    scheduleChangedAt: "2026-09-06T20:30:00",
    job: "Researches quietly every day. Reports on Mondays only — the best three of the week, or nothing.",
    ...
    //
    // She runs daily from 6 Sep 2026 but SPEAKS only on Mondays, so six of
    // every seven runs are deliberately silent. The staleness check watches
    // that she RAN, which is the thing that would actually be broken.
    quietIsNormal: true,
    acts: false,
    accent: "#8AA2B4",
  },
...
// An agent has missed the run she was last due to make. Silence looks identical
// to health, which is the failure this catches.
//
// This used to guess from the schedule text with `/day/i`, which matches the
// "day" inside "Mondays" — so every Mondays-and-Fridays agent was judged
// against a DAILY two-day threshold and the weekly branch beneath it was
// unreachable. On Sunday 6 Sep 2026 that reported Joy, Reef, Shelly and Nova as
// "gone quiet" when all four had run on Friday exactly as scheduled and were
// not due again until Monday.
//
// A panel that cries wolf every weekend is one you stop reading, which is
// exactly when the real outage lands. It now works out when the run was
// actually due — see lib/crewSchedule.js.
function isStale(run, schedule, now, scheduleChangedAt) {
  return isQuiet(run, schedule, now, undefined, scheduleChangedAt);
}

module.exports = {
  toSpokenForm, CREW, AGENT_STATUS, STATUS_KIND, isStatusRow, crewInitials, latestRun, latestStatus, statusLines, isToday, isStale, isStalled, nextDueRun };
